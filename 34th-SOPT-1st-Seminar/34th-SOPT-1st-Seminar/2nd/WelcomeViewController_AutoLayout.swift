...
class WelcomeViewController_AutoLayout: UIViewController {
...
//        let button = UIButton(frame: CGRect(x: 20, y: 498, width: 335, height: 58))
    ...
    private func setLayout() {
        NSLayoutConstraint.activate([
            rabbitImageView.topAnchor.constraint(equalTo: self.view.topAnchor, constant: 80),
            rabbitImageView.centerXAnchor.constraint(equalTo: self.view.centerXAnchor),
        ])
        
        NSLayoutConstraint.activate([
            welcomeLabel.topAnchor.constraint(equalTo: self.rabbitImageView.bottomAnchor, constant: 58),
            welcomeLabel.centerXAnchor.constraint(equalTo: self.view.centerXAnchor)
        ])
        
        NSLayoutConstraint.activate([
            goToMainButton.topAnchor.constraint(equalTo: self.welcomeLabel.bottomAnchor, constant: 71),
            goToMainButton.leftAnchor.constraint(equalTo: self.view.leftAnchor, constant: 20),
            goToMainButton.rightAnchor.constraint(equalTo: self.view.rightAnchor, constant: -20),
            goToMainButton.heightAnchor.constraint(equalToConstant: 58)
        ])
        
        NSLayoutConstraint.activate([
            retryLoginButton.topAnchor.constraint(equalTo: self.goToMainButton.bottomAnchor, constant: 14),
            retryLoginButton.leftAnchor.constraint(equalTo: self.view.leftAnchor, constant: 20),
            retryLoginButton.rightAnchor.constraint(equalTo: self.view.rightAnchor, constant: -20),
            retryLoginButton.heightAnchor.constraint(equalToConstant: 58)
        ])
    }
